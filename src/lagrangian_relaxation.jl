...
function lagrangian_heuristic(θ::AbstractArray; inst::TwoStageSpanningTreeInstance)
    ...
    # TODO Leo: look if we can improve by taking, instead of the indicator, the average values in the MST weights
end
...
function ChainRulesCore.rrule(
    ::typeof(lagrangian_dual), θ::AbstractArray; inst::TwoStageSpanningTreeInstance
)
    value, grad = lagrangian_function_value_gradient(inst, θ)
    lagrangian_dual_pullback(dl) = (NoTangent(), grad * dl, NoTangent())
    return value, lagrangian_dual_pullback
end

function lagrangian_dual_stochastic_value_grad(
    θ::AbstractArray; inst::TwoStageSpanningTreeInstance
)
    value, gs = first_stage_optimal_solution(inst, θ)

    i = 1 + (abs(rand(Int, 1)[1]) % inst.nb_scenarios)
    gs_i = zeros(ne(inst.g), inst.nb_scenarios)
    value_i = second_stage_optimal_solution(inst, θ, i, gs_i)
    value += inst.nb_scenarios * value_i
    gs += inst.nb_scenarios * gs_i
...
"""
    ...
    return (value of the solution computed),(edges in the solution computed)
"""
function lagrangian_dual_stochastic(θ::AbstractArray; inst::TwoStageSpanningTreeInstance)
    value, _ = lagrangian_dual_stochastic_value_grad(θ; inst=inst)
    return value
end

function ChainRulesCore.rrule(
    ::typeof(lagrangian_dual_stochastic),
    θ::AbstractArray;
    inst::TwoStageSpanningTreeInstance,
)
    value, gs = lagrangian_dual_stochastic_value_grad(θ; inst=inst)
    lagrangian_dual_pullback(dl) = (NoTangent(), gs * dl, NoTangent())
    return value, lagrangian_dual_pullback
end
